...
Audit du code local le 26 août 2026 : Core `0.2.0`, puis Sites `0.2.0`
sur la base `1ca3dbc`.
...
- **Partiel Sites** : des fonctions sont fusionnées dans Sites 0.2.0, pas nécessairement toute la variante autonome ni l'original tiers.
...
| 18 | SponsorBlock Seek Bar Only / Sites | Module facultatif `youtube-sponsorblock` dans WA Sites 0.2.0 : requête unique mise en cache + repères | Partiel Sites, candidat iPhone | Compatibilité réelle de la barre mobile à valider ; pas de saut automatique | Aucun appel avant activation/hors `/watch` ; repères réversibles ; lecture inchangée |
...
2. **Livré en code candidat** : Sites 0.2.0, dix modules Amazon/Google/YouTube/Reddit avec routeur, interrupteurs persistants et comparaison réversible. Les filtres restent locaux ; seul SponsorBlock, désactivé par défaut, appelle son API après activation. Aucun changement de la branche Amazon stable. Essai Safari réel à faire : [procédure et limites](wa-sites-v0.1.0.md).
